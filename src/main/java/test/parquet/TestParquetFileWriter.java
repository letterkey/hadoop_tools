...
//				"target/test/TestParquetFileWriter/testParquetFile")
//				.getAbsoluteFile();
//		testFile.delete();
//
...
//				.parseMessageType("message m { required group a {required binary b;} required group c { required int64 d; }}");
//		String[] path1 = { "a", "b" };
...
//		assertEquals(c1Ends - c1Starts, readFooter.getBlocks().get(0)
//				.getColumns().get(0).getTotalSize());
//		assertEquals(c2Ends - c2Starts, readFooter.getBlocks().get(0)
//				.getColumns().get(1).getTotalSize());
//		assertEquals(c2Ends - c1Starts, readFooter.getBlocks().get(0)
//				.getTotalByteSize());
//		HashSet<Encoding> expectedEncoding = new HashSet<Encoding>();
//		expectedEncoding.add(PLAIN);
//		expectedEncoding.add(BIT_PACKED);
//		assertEquals(expectedEncoding, readFooter.getBlocks().get(0)
//				.getColumns().get(0).getEncodings());
//
//		{ // read first block of col #1
//			ParquetFileReader r = new ParquetFileReader(configuration, path,
...
//
//			ParquetFileReader r = new ParquetFileReader(configuration, path,
//					readFooter.getBlocks(), Arrays.asList(
...
//		long[] longArray = new long[] { 39L, 99L, 12L, 1000L, 65L, 542L,
//				2533461316L, -253346131996L, Long.MAX_VALUE, Long.MIN_VALUE };
//		LongStatistics parquetMRstats = new LongStatistics();
//
//		for (long l : longArray) {
//			parquetMRstats.updateStats(l);
//		}
//		Statistics thriftStats = parquet.format.converter.ParquetMetadataConverter
//				.toParquetStatistics(parquetMRstats);
//		LongStatistics convertedBackStats = (LongStatistics) parquet.format.converter.ParquetMetadataConverter
...
//		assertEquals(parquetMRstats.getNumNulls(),
//				convertedBackStats.getNumNulls());
//	}
//
//	@Test
...
//		w.startBlock(3);
//		w.startColumn(c1, 5, codec);
//		w.writeDataPage(2, 4, BytesInput.from(bytes1), statsB1C1P1, BIT_PACKED,
//				BIT_PACKED, PLAIN);
//		w.writeDataPage(3, 4, BytesInput.from(bytes1), statsB1C1P2, BIT_PACKED,
//				BIT_PACKED, PLAIN);
//		w.endColumn();
//		w.startColumn(c2, 6, codec);
//		w.writeDataPage(3, 4, BytesInput.from(bytes2), statsB1C2P1, BIT_PACKED,
//				BIT_PACKED, PLAIN);
//		w.writeDataPage(1, 4, BytesInput.from(bytes2), statsB1C2P2, BIT_PACKED,
//				BIT_PACKED, PLAIN);
//		w.endColumn();
//		w.endBlock();
//
//		w.startBlock(4);
//		w.startColumn(c1, 7, codec);
//		w.writeDataPage(7, 4, BytesInput.from(bytes3), statsB2C1P1, BIT_PACKED,
//				BIT_PACKED, PLAIN);
//		w.endColumn();
//		w.startColumn(c2, 8, codec);
//		w.writeDataPage(8, 4, BytesInput.from(bytes4), statsB2C2P1, BIT_PACKED,
//				BIT_PACKED, PLAIN);
//		w.endColumn();
//		w.endBlock();
//		w.end(new HashMap<String, String>());
...
//			BinaryStatistics bsout = (BinaryStatistics) readFooter.getBlocks()
//					.get(0).getColumns().get(0).getStatistics();
//			String str = new String(bsout.getMaxBytes());
//			String str2 = new String(bsout.getMinBytes());
//
//			assertTrue(((BinaryStatistics) readFooter.getBlocks().get(0)
//					.getColumns().get(0).getStatistics()).equals(bs1));
//			assertTrue(((LongStatistics) readFooter.getBlocks().get(0)
//					.getColumns().get(1).getStatistics()).equals(ls1));
//		}
//		{ // assert stats are correct for the second block
//			assertTrue(((BinaryStatistics) readFooter.getBlocks().get(1)
//					.getColumns().get(0).getStatistics()).equals(bs2));
//			assertTrue(((LongStatistics) readFooter.getBlocks().get(1)
//					.getColumns().get(1).getStatistics()).equals(ls2));
//		}
//	}
//
//	@Test
...
//				"target/test/TestParquetFileWriter/testMetaDataFileDir")
//				.getAbsoluteFile();
//
...
//		List<Footer> footers = ParquetFileReader.readFooters(configuration,
//				outputStatus, false);
//		validateFooters(footers);
//		ParquetFileWriter
//				.writeMetadataFile(configuration, testDirPath, footers);
//
//		footers = ParquetFileReader.readFooters(configuration, outputStatus,
//				false);
//		validateFooters(footers);
//		footers = ParquetFileReader.readFooters(configuration,
//				fs.getFileStatus(new Path(testDirPath, "part0")), false);
//		assertEquals(1, footers.size());
//
//		final FileStatus metadataFile = fs.getFileStatus(new Path(testDirPath,
//				ParquetFileWriter.PARQUET_METADATA_FILE));
//		final FileStatus metadataFileLight = fs.getFileStatus(new Path(
//				testDirPath, ParquetFileWriter.PARQUET_COMMON_METADATA_FILE));
//		final List<Footer> metadata = ParquetFileReader.readSummaryFile(
...
//
//		footers = ParquetFileReader
//				.readAllFootersInParallelUsingSummaryFiles(configuration,
...
//		assertTrue((readFooter.getBlocks().get(0).getColumns().get(0)
//				.getStatistics().isEmpty()) == false);
//		// assert the number of nulls are correct for the first block
//		assertEquals(1, (readFooter.getBlocks().get(0).getColumns().get(0)
//				.getStatistics().getNumNulls()));
//	}
//
...
//			final Map<String, String> keyValueMetaData = parquetMetadata
//					.getFileMetaData().getKeyValueMetaData();
//			assertEquals("bar", keyValueMetaData.get("foo"));
//			assertEquals(footer.getFile().getName(),
//					keyValueMetaData.get(footer.getFile().getName()));
//		}
//	}
//
//	private void createFile(Configuration configuration, Path path,
...
//	}
//
//	private void validateContains(MessageType schema, PageReadStore pages,
//			String[] path, int values, BytesInput bytes) throws IOException {
//		PageReader pageReader = pages.getPageReader(schema
//				.getColumnDescription(path));
//		DataPage page = pageReader.readPage();
//		assertEquals(values, page.getValueCount());
//		assertArrayEquals(bytes.toByteArray(), ((DataPageV1) page).getBytes()
//				.toByteArray());
//	}
//
//	@Test
//	public void testMergeMetadata() {
//		FileMetaData md1 = new FileMetaData(new MessageType("root1",
//				new PrimitiveType(REPEATED, BINARY, "a"), new PrimitiveType(
//						OPTIONAL, BINARY, "b")), new HashMap<String, String>(),
//				"test");
//		FileMetaData md2 = new FileMetaData(new MessageType("root2",
...
//		assertEquals(merged.getSchema(), new MessageType("root1",
//				new PrimitiveType(REPEATED, BINARY, "a"), new PrimitiveType(
//						OPTIONAL, BINARY, "b"), new PrimitiveType(REQUIRED,
//						BINARY, "c")));
//
//	}
//
...
//				new ParquetMetadata(new FileMetaData(new MessageType("root1",
//						new PrimitiveType(REPEATED, BINARY, "a"),
//						new PrimitiveType(OPTIONAL, BINARY, "b")),
//						new HashMap<String, String>(), "test"), oneBlocks));
//
//		Footer two = new Footer(new Path("/tmp/output/two.parquet"),
//				new ParquetMetadata(new FileMetaData(new MessageType("root2",
//						new PrimitiveType(REQUIRED, BINARY, "c")),
//						new HashMap<String, String>(), "test2"), twoBlocks));
//
...
//
//		assertEquals(new MessageType("root1", new PrimitiveType(REPEATED,
//				BINARY, "a"), new PrimitiveType(OPTIONAL, BINARY, "b"),
//				new PrimitiveType(REQUIRED, BINARY, "c")), merged

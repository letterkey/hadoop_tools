...
public class TrainingBloomData {
	...
	public static void main(String[] args) throws IOException {
		...
		Path inputFile = new Path("hdfs://master:9000/usr/input/bloom/2.txt");

		int numMembers = Integer.parseInt("10");

		float falsePosRate = Float.parseFloat("0.01");

		Path bfFile = new Path("hdfs://master:9000/usr/input/bloom/2.bin");

		FileSystem fss = bfFile.getFileSystem(conf);
		
		fss.delete(bfFile, false);
		...
		for (FileStatus status : fss.listStatus(inputFile)) {
			BufferedReader rdr;
			// if file is gzipped, wrap it in a GZIPInputStream
			if (status.getPath().getName().endsWith(".gz")) {
				rdr = new BufferedReader(new InputStreamReader(
						new GZIPInputStream(fss.open(status.getPath()))));
			} else {
				rdr = new BufferedReader(new InputStreamReader(fss.open(status
						.getPath())));
			}
		...
		FSDataOutputStream strm = fss.create(bfFile);
		filter.write(strm);
		strm.flush();
